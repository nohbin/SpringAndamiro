...
<div class="container border mt-3 rounded-3 w-50">
		<ul class="nav nav-pills nav-fill row">
			...
			<a class="nav-link active" style="color: black;" aria-current="page" href='<c:url value="/diet/recDiet"/>'>
				추천식단
			</a>
			</li>
			<li class="nav-item col-12 col-md-3"><a class="nav-link" style="color: black;" aria-current="page" href='<c:url value="/diet/simDiet"/>'>
				간단식단
			</a>
			</li>
			<li class="nav-item col-12 col-md-3"><a class="nav-link" style="color: black;" aria-current="page" href='<c:url value="/diet/lowDiet"/>'>
				저칼로리식단
				</a>
			</li>
			<li class="nav-item col-12 col-md-3"><a class="nav-link" style="color: black;" aria-current="page"href='<c:url value="/diet/proDiet"/>'>
				프로틴식단
			</a>
			</li>
		</ul>
				 ...
				 <c:forEach var="diet" items="${diets}">
					<div class="col-12 col-md-4">
						<div class="text-center">
							...
							<div class="dietImg">
								<img src="/resources/upload/${diet.diet_picture}">			
							</div>
						</div>
					</div>
	 			  </c:forEach> 
	 			  <table>
	 			  	<tr>
	 			  		<td>
	 			  		<c:forEach var="foodName" items="${names1}">
							<div>
								<div>
									<div class="fw-bold lh-base" style="margin-left:10rem;">
										${foodName.foodName}</div>
								</div>
							</div>
						</c:forEach>
						</td>
	 			  		<td>
	 			  		<c:forEach var="foodName" items="${names2}">
							<div>
								<div>
									<div class="fw-bold lh-base" style="margin-left:10rem;">
										${foodName.foodName}</div>
	 			  		...
	 			  		<td>
	 			  		<c:forEach var="foodName" items="${names3}">
							<div>
								<div>
									<div class="fw-bold lh-base" style="margin-left:10rem;">
										${foodName.foodName}</div>
		...
		<div class="container">
			...
			<table class="table table-hover" style="height: 300px;">
				...
				<tbody class="table-secondary">
					...
					<tr>
						<td class="h5 border-end">${detail.foodName}</td>
						<td class="h5 border-end">${detail.kcal}</td>
						<td class="h5">${detail.component}</td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
